...
class Command(VerifyBaseCommand):
    """
    Verify that all important politicians do have a picture
    List the exceptions
    """
    help = "Check that all major politicians have a picture."
    ...
    def execute_verification(self, *args, **options):
        ...
        qs = OpInstitutionCharge.objects.using('politici').filter(
            ...
            politician__picture__isnull=True
        )
        ...
        qs = qs.exclude(
            Q(institution__name__iexact='consiglio comunale') |
            Q(institution__name__iexact='giunta comunale') & (
                Q(charge_type__name__iexact='assessore') |
                Q(charge_type__name__iexact='vicesindaco') |
                Q(charge_type__name__iexact='Vicesindaco facente funzione sindaco')
            )
        )
